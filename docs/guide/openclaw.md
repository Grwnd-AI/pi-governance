...
export PI_GOV_ROLE=report_author
export PI_GOV_ORG_UNIT=engineering
```
...
When deploying via OpenClaw channels, set `PI_GOV_USER` and `PI_GOV_ROLE` in your OpenClaw deployment config, or map channel user IDs in the users file:

...
| Variable                   | Purpose               |
| -------------------------- | --------------------- |
| `PI_GOV_ROLE`              | Agent role assignment |
| `PI_GOV_USER`              | User identity         |
| `PI_GOV_ORG_UNIT`          | Organization unit     |
| `PI_GOV_GOVERNANCE_CONFIG` | Custom config path    |

### Recommendations

- **Do not export secrets into the agent's environment.** Use a secrets manager or inject at process startup only.
- **Set `PI_GOV_ROLE` in the process launcher**, not in `.bashrc` or `.zshrc` where the agent could modify it.
- **Use `PI_GOV_GOVERNANCE_CONFIG`** to point to a read-only config location outside the working directory.
...
- [ ] Set `PI_GOV_ROLE` in the process launcher, not in shell config

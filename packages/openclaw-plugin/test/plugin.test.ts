...
describe('openclaw-governance plugin', () => {
  ...
  const envKeys = ['PI_GOV_USER', 'PI_GOV_ROLE', 'PI_GOV_ORG_UNIT'] as const;
  ...
  it('sets env vars from WhatsApp DM session key', () => {
    ...
    expect(process.env.PI_GOV_USER).toBe('whatsapp:+15550123');
    expect(process.env.PI_GOV_ROLE).toBe('report_author');
    expect(process.env.PI_GOV_ORG_UNIT).toBe('field-ops');
  });
  ...
  it('uses default role for unknown user', () => {
    ...
    expect(process.env.PI_GOV_USER).toBe('whatsapp:+19999999');
    expect(process.env.PI_GOV_ROLE).toBe('analyst');
    expect(process.env.PI_GOV_ORG_UNIT).toBe('default');
  });
  ...
  it('does not set env vars for unparseable session key', () => {
    ...
    expect(process.env.PI_GOV_USER).toBeUndefined();
    expect(process.env.PI_GOV_ROLE).toBeUndefined();
    expect(process.env.PI_GOV_ORG_UNIT).toBeUndefined();
  });
  ...
  it('resolves Discord user correctly', () => {
    ...
    expect(process.env.PI_GOV_USER).toBe('discord:428374928374');
    expect(process.env.PI_GOV_ROLE).toBe('admin');
    expect(process.env.PI_GOV_ORG_UNIT).toBe('platform');
  });
});
